...
println("ketcindylibbasic3[20260225] loaded");
...
//help:Usetex("lua"[,"p",""]);
 //global PathT
  regional(full,str);
  full=name;
  if(indexof(full,"latex")==0,
      full=full+"latex";
  );
  PathT=PathThead+"/"+full;
);
////%Usetex end////
...
//help:Maketitle();
  ...
  println(SCEOUTPUT,"\geometry{paperwidth=128mm,paperheight=91mm}");
  ...
  println(SCEOUTPUT,"\usepackage{graphicx}");
  println(SCEOUTPUT,"\usepackage{color}");
  println(SCEOUTPUT,"\setmargin{1}{1}{1}{1}");
...
////%Presentation start//// 260227 changed by kubo
...
//help:Presentation(texfile,txtfile);
  ...
  flgL=[];
  SCEOUTPUT = openfile(file);// 260227 use later for txL by kubo
  ...
  println(SCEOUTPUT,"\documentclass{article}");
  println(SCEOUTPUT,"\usepackage{geometry}");
  println(SCEOUTPUT,"\geometry{paperwidth=148mm,paperheight=105mm}");
  println(SCEOUTPUT,"\usepackage{luatexja}");
  println(SCEOUTPUT,"\usepackage{pict2e}");
  println(SCEOUTPUT,"\usepackage{ketpic2e,ketlayer2e}");
  println(SCEOUTPUT,"\usepackage{ketslide}");
...
//  );
  ...
  println(SCEOUTPUT,"\setmargin{9}{9}{3}{-17}");
  ...
  forall(2..length(slideL),ns,
    ...
    if((substring(sld,0,1)=="%") & (substring(sld,0,2)!="%%"), // 17.06.23
      ...
      if(substring(sld,1,5)=="para",
        ...
        LinkPosH=125;LinkPosV=70;//260225 add by kubo
